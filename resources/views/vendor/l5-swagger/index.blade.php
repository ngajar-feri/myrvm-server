    ...
    window.onload = function() {
        ...
        const ui = SwaggerUIBundle({
            ...
            responseInterceptor: function(response) {
                if (response.url.endsWith('/api/v1/login') && response.status === 200) {
                    try {
                        const responseData = JSON.parse(response.data);
                        // Adjust this path based on your actual response structure
                        // Example structure: { "data": { "token": "..." } } or { "token": "..." }
                        // Based on user feedback: response.data.token
                        const token = responseData.data?.token || responseData.token;

                        if (token) {
                            const bearerToken = 'Bearer ' + token;
                            console.log('Swagger UI: Auto-filling Authorize with token');
                            
                            // Key 'bearerAuth' MUST match the security scheme name in api-docs.json
                        ...
                        console.error('Swagger UI: Failed to auto-authorize', e);
                    }
                }
                return response;
            },
